...
LIVE = ("program", "user_slot", "save", "in_gain", "output", "fx", "fx_2",
        "tap", "mute", "ab", "slot_name", "next_user")

# The words a USER slot can be called on the device screen.
#
# A control port carries a number and not text, so a name typed in a
# browser cannot reach the plugin at all - it stayed in that browser, which
# is exactly as useful as not having it. A word PICKED from a list is a
# number, travels into the slot, is saved with the pedalboard, and can be
# read out on the pedal. Eight upper-case characters at most: the device
# truncates silently.
SLOT_WORDS = (
    "USER", "INTRO", "VERSE", "CHORUS", "BRIDGE", "SOLO", "BREAK", "ENDING",
    "SONG 1", "SONG 2", "SONG 3", "SONG 4", "BALLAD", "ROCK", "POP", "FUNK",
    "JAZZ", "BLUES", "GOSPEL", "COUNTRY", "METAL", "REGGAE", "SPEECH",
    "CHOIR", "DOUBLE", "HARMONY", "EFFECT", "SOFT", "LOUD", "DRY", "WET",
    "SPARE",
)
...
# Ports added AFTER the outputs, and never anywhere else. A port index is
# how a pedalboard remembers which control is which: inserting one in the
# middle silently hands every later value to its neighbour. Everything
# added from build 12 on goes here, in order, whatever its direction.
TAIL = [
 ("in", "enc_slot", "ENC 1 FAVOURITE", 0.0, 1.0, 0.5, None, [],
  "Encoder 1 of the pedal page. Read in DETENTS, never as a position: the "
  "plugin counts the clicks of the knob, so a control that changes meaning "
  "never makes a value jump. Turning it walks the USER slots and the sound "
  "follows; while a name is being typed it moves the cursor along the word."),
 ("in", "enc_param", "ENC 2 PARAMETER", 0.0, 1.0, 0.5, None, [],
  "Encoder 2. Walks the parameters of the sound in force - NAME first, then "
  "everything a program owns - and shows the one it points at with its "
  "value. While a name is being typed it is the letter under the cursor."),
 ("in", "enc_value", "ENC 3 VALUE", 0.0, 1.0, 0.5, None, [],
  "Encoder 3. Changes the parameter encoder 2 points at, one detent at a "
  "time, and the value is written back to its own port so the web page and "
  "the pedalboard follow. On NAME it opens the name editor; inside the "
  "editor, left cancels and right keeps."),
 ...
 # TWO properties, and the plugin measures a press only because of them.
 #
 # NOT a pprops:trigger: a trigger is by MOD's own convention a momentary
 # PULSE, the host writing one and then zero whatever the foot does, so
 # the length of a press cannot be seen from in here - and mod-ui, knowing
 # it is a trigger, offers no Momentary option at all, there being nothing
 # to choose.
 #
 # AND mod:preferMomentaryOnByDefault, which is what makes an addressed
 # footswitch follow the foot: high while it is down, low when it lifts.
 # Without it the addressing defaults to a LATCH - the port goes high on
 # one press and stays there until the next - and then a hold is a switch
 # left on, indistinguishable from a foot that never lifted. Every switch
 # of the plugin this was learnt from carries it.
 #
 # Every other switch here stays a trigger: a pulse is exactly right for
 # something that acts on the press and has no duration to read.
 ("in", "fav_browse", "FAV BROWSE", 0.0, 1.0, 0.0, None,
  ["lv2:toggled", "mod:preferMomentaryOnByDefault"],
  ...
  "so walking fast is one step per press with nothing to wait for. It asks "
  "to be addressed MOMENTARY ON by default, which is what makes the port "
  "follow the foot instead of latching - and it is a plain toggle rather "
  "than a trigger for the same reason, a trigger being a pulse whose "
  "length cannot be seen. DIAG says whether the hold arrives. "
...
# eight upper-case characters. Only the ones that do not fit are here.
SHORT_NAME = {
    "de_ess_freq": "SS FREQ", "mid_freq": "MID HZ", "delay_time": "TIME",
    "delay_repeats": "REPEATS", "delay_mix": "DLY MIX", "reverb_mix": "REV MIX",
    "pitch_mix": "PIT MIX", "modulation": "MOD", "mod_speed": "SPEED",
    "harm_mix": "HARMONY", "doubler": "DOUBLE", "feedback": "NO HOWL",
    "low_cut": "LOW CUT", "presence": "PRESENCE",
}

# How much one detent of encoder 3 moves a control. Anything not named
# here moves by one hundredth of its range, which suits a per-cent knob.
STEP = {
    "low_cut": 5.0, "gate": 1.0, "mid_freq": 50.0, "de_ess_freq": 100.0,
    "body": 0.5, "presence": 0.5, "air": 0.5, "pitch": 1.0, "harm_1": 1.0,
...
SCALE = {
    ...
    "slot_name": [(w.title() if w != "USER" else "No name", float(i))
                  for i, w in enumerate(SLOT_WORDS)],
}
...
def write(path, uri, name, label, audio):
    ...
    for way, symbol, pname, mn, mx, default, unit, props, comment in TAIL:
        blocks.append(port_block(index, "ControlPort",
                                 "InputPort" if way == "in" else "OutputPort",
                                 symbol, pname, comment, mn, mx, default, unit,
                                 props))
        index += 1
...
def write_programs(path):
    ...
    # what the pedal page shows for each parameter it walks
    unite = {"db": "DB", "hz": "HZ", "pc": "%", "ms": "MS", "s": "S"}
    spec = ""
    for c in cont:
        symbol, pname, mn, mx, _d, unit, _p, _cm = c
        nom = SHORT_NAME.get(symbol, pname)
        if len(nom) > 8 or nom != nom.upper():
            raise SystemExit("screen name %r for %s must be 8 upper-case "
                             "characters or fewer - add it to SHORT_NAME"
                             % (nom, symbol))
        pas = STEP.get(symbol, (mx - mn) / 100.0)
        dec = 0 if pas >= 1.0 else (1 if pas >= 0.1 else 2)
        # the C name of a control is not always its symbol shouted
        c_nom = {"modulation": "MOD", "de_ess": "DE_ESS",
                 "de_ess_freq": "DE_ESS_FREQ", "doubler": "DOUBLER"}.get(
                     symbol, symbol.upper())
        spec += ('    { CTL_%s, "%s", "%s", %sf, %d },\n'
                 % (c_nom, nom, unite.get(unit, ""), fmt(pas), dec))

    mots = "".join('    "%s",\n' % w for w in SLOT_WORDS)
    for w in SLOT_WORDS:
        if len(w) > 8 or w != w.upper():
            raise SystemExit("slot word %r must be 8 upper-case characters "
                             "or fewer" % w)
